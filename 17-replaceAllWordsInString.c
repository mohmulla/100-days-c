...
int main()
{
	char string[81] = { "hello beautiful beautiful world" };
	char s1[81] = { " " };
	char s2[81] = { "" };

	bool stillFound = false;

	bool ReplaceString(char sourceString[], char stringToBeReplaced[], char replacementString[]) ;
	
	do
	{
		stillFound = ReplaceString(string, s1, s2);
	}
	while (stillFound);
